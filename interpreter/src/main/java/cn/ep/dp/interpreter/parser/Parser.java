package cn.ep.dp.interpreter.parser;

import cn.ep.dp.interpreter.expression.*;
...
public class Parser {
     ...
     * @return 对应的抽象语法树
     */
    public static BaseXmlExpression parse(String expr) {
        //先初始化记录需解析的元素的名称的集会
        ...
        List<BaseXmlExpression> list = mapPath2Interpreter(mapPath);
    ...
    private static Map<String, ParserModel> parseMapPath(String expr) {
        ...
        //初始化一个map用来存放分解出来的值
        Map<String, ParserModel> mapPath = new HashMap<>(16);
     ...
     */
    private static List<BaseXmlExpression> mapPath2Interpreter(Map<String, ParserModel> mapPath) {
        List<BaseXmlExpression> list = new ArrayList<>();
        //一定要按照分解的先后顺序来转换成解释器对象
        for (String key : listEle) {
            ParserModel pm = mapPath.get(key);
            BaseXmlExpression obj;
            if (!pm.isEnd()) {
                if (pm.isSingleVlaue()) {
                    //不是最后一个，是一个值，转化为
    ...
    private static BaseXmlExpression buildTree(List<BaseXmlExpression> list) {
        //第一个对象，也是返回去的对象，就是抽象语法树的根
        BaseXmlExpression returnRe = null;
        //定义上一个对象
        BaseXmlExpression preRe = null;
        for (BaseXmlExpression re : list) {
            if (preRe == null) {
                //说明是第一个元素
                preRe = re;
                returnRe = re;
